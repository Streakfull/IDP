...
class OSCSiameseNetworkFeaturesKPTransformer(nn.Module):

    def __init__(self) -> None:
        super(OSCSiameseNetworkFeaturesKPTransformer, self).__init__()

        self.osc = self.load_osc()
...
# Count non-zero keypoints
       ...
       # print(f"Number of non-zero keypoint tensors in batch: {num_nonzero}")
        ...
        features = self.pose(keypoints_tensor)  # Shape: (num_objects, 512)
        return features
