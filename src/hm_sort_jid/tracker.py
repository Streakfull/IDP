...
from hm_sort_jid import matching
from hm_sort_jid.basetrack import BaseTrack, TrackState
from hm_sort_jid.kalman_filter import KalmanFilter
from hm_sort_jid.Strack import STrack
...
class Deep_EIoU(object):
    ...
    def adjust_dist(self, emb_dists, ious_dists, tracks, det):
        ious_dists_mask = (ious_dists > self.proximity_thresh)
        raw_embed = emb_dists.copy()
        emb_dists[ious_dists_mask] = 1.0
        emb_dists[emb_dists > self.appearance_thresh] = 1.0
        dists = (1/(ious_dists+np.finfo(float).eps)) + \
            (1/(emb_dists+np.finfo(float).eps))
        dists = 2 / dists
        # dists = np.minimum(ious_dists, emb_dists)
        # dists[emb_dists > self.appearance_thresh] = 1.0
        dists = self.adjust_hard_jersey_id(dists, tracks, det)
        return dists

    def adjust_hard_jersey_id(self, dist_matrix, tracks, det):
        for i in range(len(tracks)):
            for k in range(len(det)):
                t = tracks[i]
                di = det[k]
                if (t.jid == di.jid and t.jid is not None):
                    dist_matrix[i, :] = 1
                    dist_matrix[i, k] = 0
                    # print("JID match:", t.jid)
        return dist_matrix

    # def adjust_hard_jersey_id(self, dist_matrix, tracks, det):
    #     # Extract JIDs and filter out None values
    #     track_jids = np.array(
    #         [t.jid if t.jid is not None else -1 for t in tracks])
    #     # Ensure different default
    #     det_jids = np.array([d.jid if d.jid is not None else -2 for d in det])

    #     # Find matches
    #     # Shape: (num_tracks, num_det)
    #     valid_mask = track_jids[:, None] == det_jids

    #     # Update the distance matrix correctly
    #     dist_matrix[valid_mask] = 0  # Set (i, k) to 0 where JID matches
    #     # Tracks that have at least one match
    #     rows_with_match = np.any(valid_mask, axis=1)
    #     # Set full row to 1 for those tracks
    #     dist_matrix[rows_with_match, :] = 1

    #     print("JID matches:", track_jids[rows_with_match])
    #     return dist_matrix

    def update(self, output_results, embedding, jids):
        '''
        ...
        if len(output_results):
            ...
            if self.args.with_reid:
                ...
                jids = jids[lowest_inds]
                features_keep = embedding[remain_inds]
                jids_keep = jids[remain_inds]
        ...
        if len(dets) > 0:
            '''Detections'''
            if self.args.with_reid:
                detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, f, jid=jid) for
                              (tlbr, s, f, jid) in zip(dets, scores_keep, features_keep, jids_keep)]
            else:
        ...
        for iteration in range(num_iteration):
            ...
            if self.args.with_reid:
                ...
                dists = self.adjust_dist(
                    emb_dists, ious_dists, strack_pool, detections)
            else:
                dists = ious_dists
        ...
        if len(dets_second) > 0:
            '''Detections'''
            if self.args.with_reid:
                detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, f, jid=jid) for
                                     (tlbr, s, f, jid) in zip(dets_second, scores_second, features_second, jids_second)]
            else:
        ...
        if self.args.with_reid:
            ...
            dists = self.adjust_dist(
                emb_dists, ious_dists, unconfirmed, detections)
