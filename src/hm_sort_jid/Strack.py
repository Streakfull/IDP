...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, feat=None, feat_history=30, jid=None):
    ...
    def update_features(self, feat):
        ...
        if self.smooth_feat is None:
            ...
            cosine_sim = np.dot(self.smooth_feat, feat) / \
                (np.linalg.norm(self.smooth_feat) * np.linalg.norm(feat))
            cosine_dist = 1 - cosine_sim

            # Set alpha adaptively
            if cosine_dist > 0.1:
                alpha = 1.0
            else:
                alpha = cosine_dist / 0.1  # Inversely proportional scaling

            # Apply update rule
            self.smooth_feat = alpha * self.smooth_feat + (1 - alpha) * feat
    #     self.smooth_feat /= np.linalg.norm(self.smooth_feat)  # Normalize

    def update_features_direct(self, feat):
        self.curr_feat = feat
        self.smooth_feat = feat
        # print("UPDATING HERE???")
