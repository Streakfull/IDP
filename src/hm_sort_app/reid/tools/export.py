...
from hm_sort.reid.torchreid.utils.feature_extractor import FeatureExtractor
from hm_sort.reid.torchreid.models import build_model
...
def export_tflite(file, half):
    ...
    try:
        import openvino.inference_engine as ie
        print(f'\nStarting export with openvino {ie.__version__}...')
        output = Path(str(file).replace(
            f'_openvino_model{os.sep}', f'_tflite_model{os.sep}'))
        ...
        print(
            f'Export success, results saved in {output} ({file_size(f):.1f} MB)')
        return f
    except Exception as e:
        print(f'\nExport failure: {e}')
...
if __name__ == "__main__":
    ...
    im = torch.zeros(1, 3, args.imgsz[0], args.imgsz[1]).to(
        'cpu')  # image size(1,3,640,480) BCHW iDetection
    if onnx:
        f = export_onnx(extractor.model.eval(), im, args.weights, 12,
                        train=False, dynamic=args.dynamic, simplify=True)  # opset 12
    if openvino:
