...
- Results are presented in the format of _<Rank-1 (mAP)>_.
- When computing model size and FLOPs, only layers that are used at test time are considered (see `hm_sort.reid.torchreid.utils.compute_model_complexity`).
