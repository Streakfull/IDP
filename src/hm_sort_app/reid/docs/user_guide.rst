...
.. code-block:: python
    ...
    start_epoch = hm_sort.reid.torchreid.utils.resume_from_checkpoint(
...
--------------------------
We provide a tool in ``hm_sort.reid.torchreid.utils.model_complexity.py`` to automatically compute the model complexity, i.e. number of parameters and FLOPs.

.. code-block:: python
    
    from hm_sort.reid.torchreid import models, utils
...
.. code-block:: python
    ...
    datamanager =hm_sort.reid.torchreid.ImageDataManager(
        root='reid-data',
...
----------------------
You can load a trained model using :code:`hm_sort.reid.torchreid.utils.load_pretrained_weights(model, weight_path)` and set ``test_only=True`` in ``engine.run()``.


Fine-tune a model pre-trained on reid datasets
-----------------------------------------------
Use :code:`hm_sort.reid.torchreid.utils.load_pretrained_weights(model, weight_path)` to load the pre-trained weights and then fine-tune on the dataset you want.
...
.. code-block:: python
    ...
    from hm_sort.reid.torchreid import ImageDataset
...
.. code-block:: python
    ...
    import torchreid
   hm_sort.reid.torchreid.register_image_dataset('new_dataset', NewDataset)
...
.. code-block:: python
    ...
    from hm_sort.reid.torchreid.utils import FeatureExtractor
