...
import numpy as np
import random
from clipreid.loss import ClipLoss, ClipLossOG
...
class SiameseNCE(BaseModel):
    ...
    def set_loss(self):
        ...
        # self.loss = self.clip_loss(
        #     self.x1, self.x2, self.clip.model.logit_scale.exp())
    ...
    def name(self):
        return 'SiameseLargeClip'

    def build_network(self):
        model = OpenClipModel(self.configs["name"],
                              self.configs["pretrained"],
                              True)

        # state_dict = torch.load("./chkpts2/clip.pth")
        # model.load_state_dict(state_dict)
        # cprint("Clip model Loaded", color="yellow")
        return model
    ...
    def prepare_query_gallery_matrix(self, batch):
        """Organizes batch into query-gallery pairs ensuring each query is selected once and has one positive match."""
        images = batch["img"]  # Shape: [B, C, H, W]
        pids = batch["pid"]  # Shape: [B]
        ...
        for pid in unique_pids:
            ...
            # Ensure each query is selected once
            query_idx = random.choice(indices.tolist())
            while query_idx in selected_queries:
                query_idx = random.choice(indices.tolist())
            selected_queries.add(query_idx)

            I_q.append(images[query_idx])
            pids_q.append(pid)

            # Select one random positive gallery image
            remaining_indices = indices[indices != query_idx]
            gallery_idx = random.choice(remaining_indices.tolist())
            I_g.append(images[gallery_idx])
            pids_g.append(pid)

        # Convert lists to tensors
        I_q = torch.stack(I_q) if I_q else torch.empty(
            (0, *images.shape[1:]))  # [num_pairs, C, H, W]
        I_g = torch.stack(I_g) if I_g else torch.empty(
            (0, *images.shape[1:]))  # [num_pairs, C, H, W]
        pids_q = torch.tensor(pids_q, dtype=torch.long)
        pids_g = torch.tensor(pids_g, dtype=torch.long)
