...
import torch
import cv2
from hm_sort_app.tracker import BaseTrack, Deep_EIoU
from hm_sort_app.reid.torchreid.utils import FeatureExtractor
from siamese_tracking.Detection import Detection
from pytorchmodels.SiameseTriplet import TripletSiamese
from pytorchmodels.siameseNCEResnet import SiameseNCE
...
class HMTracker(ObjectDetection):
    def __init__(self, write_path, use_high_res=False, res=None, use_crops_cls=False) -> None:
    ...
    def load_siamese(self):
        ...
        # self.siamese_network = SiameseNCE(
        #     self.siamese_configs, self.global_configs["training"])
    ...
    def load_model(self):
        if (self.use_high_res and False):
    ...
    def get_detections_objects(self, det, frame, frame_path):
        ...
        crops = self.soccernet.load_frame_crops(det, frame_path)
    ...
    def get_detections_objects_file(self, det, frame, frame_path):
        ...
        with torch.no_grad():
            features = self.siamese_network.img_feature(crops)
            # features = self.extractor(crops)
            if (self.use_crops_cls):
                # cropslg = self.soccernet.load_frame_crops_det_lg(
                #     det, frame_path)
                crops_classification = self.crops_cls.inference_with_cls(
                    crops)
        ...
        for i in range(len(features)):
            ...
            if (self.use_crops_cls):
                det[i].set_classif(crops_classification[i].item())
        return det

    def get_detections_objects_file_kp(self, det, frame, frame_path):
        crops = self.soccernet.load_frame_crops_det(det, frame_path)
        self.siamese_network.eval()
        self.siamese_network.clip.eval()
        if len(crops) == 0:
        ...
        with torch.no_grad():
            ...
            # import pdb
            # pdb.set_trace()
            features = self.siamese_network.clip.forward_x1_x2(crops, kp)
        ...
        with tqdm(total=total_frames, desc="Processing frames", unit="frame") as pbar:
            for frame_idx, frame_file in enumerate(frame_files):
                ...
                detections = self.get_detections_objects_file(
                    det, img, frame_path)
                # detections = [d for d in detections if d.confidence >= 0.4]
                if self.use_crops_cls:
                    player_detections = []
                    extra_detections = []
                    for d in detections:
                        if d.is_player():
                            # import pdb
                            # pdb.set_trace()
                            player_detections.append(d)
                        else:
                            extra_detections.append(d)
                    detections = player_detections

                boxes = np.array([detection.bb for detection in detections])
                feats = np.array([detection.get_feature()
                                 for detection in detections])
                cls = np.array([detection.get_cls()
                               for detection in detections])
            ...
            if self.use_crops_cls:
                existing_tids = set()  # Keep track of used TIDs
                for et in extra_detections:
                    tlwh = et.tlwh
                    base_tid = class_id_map.get(et.get_cls(), -10)
                    suffix = 0
                    tid = base_tid * 1000 - suffix  # Keep it negative and grouped

                    # Ensure tid is unique
                    while tid in existing_tids:
                        suffix += 1
                        tid = base_tid * 1000 - suffix
                    existing_tids.add(tid)

                    result = np.concatenate(
                        (tlwh, [et.get_conf()], [et.get_cls()], [tid]))
                    text_file.append(
                        f"{frame_idx+1},{tid},{tlwh[0]:.2f},{tlwh[1]:.2f},{tlwh[2]:.2f},{tlwh[3]:.2f},{et.get_conf():.2f},-1,-1,-1\n"
                    )
    ...
    def process_frames_from_folder_fixed_bb2(self, frames_folder,
                                             write_path="./logs/outputLive/",
                                             labels_write_path=None,
                                             global_text_file_path=None,
                                             write_directly=True,
