...
class BaseModel(torch.nn.Module):
    ...
    def load_ckpt(self, ckpt_path):
        ...
        cprint(
            f"Matched keys: {len(model_state_dict) - len(missing_keys) - len(unexpected_keys)}", "green")
        cprint(f"Missing keys: {len(missing_keys)}", "yellow")
        cprint(f"Unexpected keys: {len(unexpected_keys)}", "red")
        cprint(f"{self.name()} loaded from {ckpt_path}", "blue")
