...
class TeamClustering(ObjectDetection):
    ...
    def load_model(self):
        ...
        _ = self.model(ASSETS / "bus.jpg", save=False,
                       embed=[16, 19, 22, 23], imgsz=1920, classes=0, conf=0.5)
        return model
    ...
    def predict(self, img):
        prepped = self.model.predictor.preprocess(
            [img])
        result = self.model.predictor.inference(
            prepped)
    ...
    def get_visual_crop_features(self, det, frame):
        ...
        for idx, box in enumerate(boxes):
            ...
            #     self.write_path, "crops", f"crop_{idx}_visual.jpg")
            # img_crop.save(save_path)
            img_crop = transform(img_crop)
            with torch.no_grad():
                # self.siamese_network.network.vit.eval()
    ...
    def get_crops(self, det, frame, padded=False):
        boxes = det.boxes.xyxy
        features = []
        crops = []
        for idx, box in enumerate(boxes):
            ...
            # img_crop = self.convert_pil_bgr_to_rgb(img_crop)
            crops.append(np.array(img_crop))
        return crops
    ...
    def get_detections_objects_yolo(self, det, frame):
        results = self.get_full_pred(det)
        features = det.feats
        objects = list(map(Detection, results))
        for i in range(len(features)):
    ...
    def apply_team_cluster(self, det, frame, teams):
        results = self.get_full_pred(det)
        objects = list(map(Detection, results))
        for i in range(len(det)):
            detection = objects[i]
            detection.set_id(teams[i])
        return objects
