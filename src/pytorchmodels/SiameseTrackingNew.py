    ...
    def process_frames_from_folder_fixed_bb(self, frames_folder,
        ...
        with tqdm(total=total_frames, desc="Processing frames", unit="frame") as pbar:
            for frame_idx, frame_file in enumerate(frame_files):
                ...
                detections = self.get_detections_objects_file_kp(
                    det, img, frame_path)
                ...
                for t in detections:
                    ...
                    online_scores.append(t.get_conf())
                    result = np.concatenate((tlwh, [t.get_conf()], [0], [tid]))
                    results.append(result)
                    text_file.append(
                        f"{frame_idx+1},{tid},{tlwh[0]:.2f},{tlwh[1]:.2f},{tlwh[2]:.2f},{tlwh[3]:.2f},{t.get_conf():.2f},-1,-1,-1\n"
                    )
