...
from pytorchmodels.jerseyId.strhub.data.utils import CharsetAdapter, CTCTokenizer, Tokenizer, BaseTokenizer
...
class CTCSystem(BaseSystem):
    ...
    def forward_logits_loss(self, images: Tensor, labels: List[str]) -> Tuple[Tensor, Tensor, int]:
        ...
        loss = F.ctc_loss(log_probs, targets, input_lengths,
                          target_lengths, blank=self.blank_id, zero_infinity=True)
