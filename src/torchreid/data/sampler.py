...
from clipreid.tools.prepare_lookups_for_hierarchical_sampling import create_lookupdict
...
class RandomIdentitySampler_Hierarchical(Sampler):
        ...
        if selected_pids_extra is None:
            ...
            if isinstance(some_identifier, set):
                ...
                for each_identifier in some_identifier:
                    selected_pids_extra = selected_pids_extra.union(
                        some_identifier_to_pid[each_identifier])
    ...
    def __iter__(self):
        ...
        atleast_one_team_year_league_pid = copy.deepcopy(
            self.atleast_one_team_year_league_pid)
        atleast_one_team_league_pid = copy.deepcopy(
            self.atleast_one_team_league_pid)
        ...
        while len(avai_pids) >= self.num_pids_per_batch:
            ...
            # Make a deep copy
            selected_pids = copy.deepcopy(
                action_pid[self.pid_action[selected_pid]])
            ...
            if len(selected_pids) > self.num_pids_per_batch:
                ...
                extra_pids = random.sample(selected_pids, len(
                    selected_pids) - self.num_pids_per_batch)
            ...
            if len(selected_pids) < self.num_pids_per_batch:
                ...
                team_year_league_pid[self.pid_team_year_league[selected_pid]].remove(
                    selected_pid)
                team_league_pid[self.pid_team_league[selected_pid]].remove(
                    selected_pid)
                for identifier in self.pid_atleast_one_team_year_league[selected_pid]:
                    atleast_one_team_year_league_pid[identifier].remove(
                        selected_pid)
                for identifier in self.pid_atleast_one_team_league[selected_pid]:
                    atleast_one_team_league_pid[identifier].remove(
                        selected_pid)
                continue
            ...
            for pid in selected_pids:
                ...
                if len(batch_idxs_dict[pid]) == 0:
                    ...
                    team_year_league_pid[self.pid_team_year_league[pid]].remove(
                        pid)
...
def build_train_sampler(
    ...
    **kwargs
    ...
    if train_sampler == 'RandomIdentitySampler_Hierarchical':
        sampler = RandomIdentitySampler_Hierarchical(
