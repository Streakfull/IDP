...
class ModelTrainer:
    def __init__(self, dataset_type: Type[BaseDataSet] = SimMatches, configs_path="./configs/global_configs.yaml", options: dict = {}):
        ...
        self.val_sn = self.data_loader_handler.val_sn
    ...
    def _train_one_epoch(self, epoch):
        ...
        for batch_idx, batch in enumartion:
            ...
            if iteration % self.training_config['validate_every'] == (self.training_config['validate_every'] - 1) or (
                ...
                self.model.eval()
                r1, mAP = self.model.test(self.validation_dataloader)
                ...
                for batch_index, batch_val in self.tqdm(enumerate(self.val_sn), total=len(self.val_sn)):
                    with torch.no_grad():
               ...
               # avg_iou_val = val_iou_running / index_batch
                ...
                self.logger.add_scalar(
                    f"Validation/r1", r1, iteration)
                self.logger.add_scalar(
                    f"Validation/mAP", mAP, iteration)
